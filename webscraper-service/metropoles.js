...
// Mapa de feeds RSS por categoria
const RSS_FEEDS = [
  { url: 'https://www.metropoles.com/tag/tecnologia/feed', categoria: 'tecnologia' },
  { url: 'https://www.metropoles.com/esportes/feed', categoria: 'esportes' },
  { url: 'https://www.metropoles.com/economia/feed', categoria: 'economia' },
  { url: 'https://www.metropoles.com/entretenimento/feed', categoria: 'cultura' }
];

const BASE_DOMAIN = 'https://www.metropoles.com/';
...
async function fetchRSS(feedUrl, categoria) {
  console.log(`[Metropoles Webscraper] Buscando RSS de ${categoria}: ${feedUrl}...`);
  ...
  console.log(`[Metropoles Webscraper] ${urls.length} URLs extraídas do RSS de ${categoria}`);
  return urls;
}
...
export function extractContent($) {
    ...
    for (const selector of selectors) {
      ...
      if (element.length > 0) {
        const allowedElements = element.find('p, h2, h3, h4, ul, ol, blockquote').toArray();
        const extracted = allowedElements.map((el) => $.html(el)).join('\n');

        if (extracted && extracted.length > conteudo.length) {
            conteudo = extracted;
            console.log(`[Metropoles] Content found with selector: ${selector} (${extracted.length} chars)`);
        }
      }
    }

    if (!conteudo || conteudo.length < 200) {
        const $scope = $('[class^="ConteudoNoticiaWrapper__Artigo-"]').first().length
        ? $('[class^="ConteudoNoticiaWrapper__Artigo-"]').first()
        : $('.ConteudoNoticiaWrapper-sc-19fsm27-0.hIDPRr.m-content').first();
...
function removeSplideCarousels($contentClean) {
    // Remove contêineres e estruturas do Splide (Metropoles)
    $contentClean('.splide, .splide-container, .pre-slider').remove();
    $contentClean('ul.splide__pagination, .splide__pagination, .splide__arrows, .splide__track, .splide__list, .splide__slide, .splide__close-modal, .splide__metropoles-logo, .splide__thumb-list, .splide__first').remove();
    $contentClean('[id^="splide"], [aria-roledescription="carousel"]').remove();
  }
...
function removeJupiterSummaryElements(contentHtml) {
    ...
    contentHtml = contentHtml.replace(/<div[^>]*class="[^"]*\bsplide-container\b[^"]*"[^>]*>[\s\S]*?<\/div>/gi,'');
    contentHtml = contentHtml.replace(/<(div|section)[^>]*class="[^"]*\bsplide\b[^"]*"[^>]*>[\s\S]*?<\/\1>/gi,'');
    contentHtml = contentHtml.replace(/<ul[^>]*class="[^"]*\bsplide__pagination\b[^"]*"[^>]*>[\s\S]*?<\/ul>/gi,'');
...
async function scrapePage(url) {
  ...
  removeSplideCarousels($contentClean);
...
async function createNoticia(item, url, data_publicacao, categoria) {
  ...
  if (existe) {
    console.log(`[Metropoles Webscraper] Pulando notícia duplicada: ${item.titulo.substring(0, 60)}...`);
    return 'skipped';
  }
  ...
  // Converte o nome da categoria para o ID numérico
  const categoriaId = CATEGORIAS_MAP[categoria] || CATEGORIAS_MAP['tecnologia'];
  ...
  console.log(`[Metropoles Webscraper] Criando notícia [${categoria}]: ${item.titulo.substring(0, 60)}...`);
  ...
  if (response.ok) {
    console.log('[Metropoles Webscraper] Notícia criada com sucesso!');
    return true;
  } else {
    const error = await response.text();
    console.log('[Metropoles Webscraper] ❌ Erro ao criar notícia:', error);
    return false;
  }
}
    ...
    for (const feed of RSS_FEEDS) {
      ...
      try {
        ...
        for (let i = 0; i < urls.length; i++) {
          const { url, data_publicacao, categoria } = urls[i];
          try {
            const item = await scrapePage(url);
            item.destaque = i === 0;
            const resultado = await createNoticia(item, url, data_publicacao, categoria);
            ...
            console.error(`[Metropoles Webscraper] ❌ Erro ao processar: ${error.message}`);
            erros++;
          }
        }
      } catch (error) {
        console.error(`[Metropoles Webscraper] ❌ Erro ao processar feed ${feed.categoria}: ${error.message}`);
        erros++;
      }
    }

    console.log('\n[Metropoles Webscraper] ========================================');
    console.log('[Metropoles Webscraper] Importação concluída!');
    console.log(`[Metropoles Webscraper] Total processado: ${totalProcessado}`);
    console.log(`[Metropoles Webscraper] Criadas: ${criadas}`);
    console.log(`[Metropoles Webscraper] Puladas (duplicadas): ${puladas}`);
    console.log(`[Metropoles Webscraper] Erros: ${erros}`);
    console.log('[Metropoles Webscraper] ========================================\n');

  } catch (error) {
    console.error('[Metropoles Webscraper] ❌ Erro fatal:', error);
  }
}
