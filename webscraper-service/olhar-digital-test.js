...
async function fetchRSS(feedUrl, categoria) {
  try {
    ...
    // Limitar a 3 artigos para teste
    const limitedUrls = urls.slice(0, 3);
    console.log(`[Olhar Digital Test] ${limitedUrls.length} URLs extraídas (limitado a 3)`);
    return limitedUrls;
  } catch (error) {
    console.error('[Olhar Digital Test] ❌ Erro ao buscar RSS:', error.message);
    return [];
  }
}
...
async function scrapePage(url) {
  try {
    console.log(`[Olhar Digital Test] Fazendo scraping: ${url}`);
    ...
    if (article.length > 0) {
      ...
      console.log(`[Olhar Digital Test] Conteúdo com article (${conteudo.length} chars)`);
    }

    // Se não encontrou article, usar fallback
    if (!conteudo || conteudo.length < 1000) {
      console.log('[Olhar Digital Test] Conteúdo curto, usando fallback');
      const paragraphs = $('p, h2, h3, h4, ul, ol, blockquote').toArray();
      conteudo = paragraphs.map(p => $.html(p)).join('\n');
      console.log(`[Olhar Digital Test] Fallback: ${conteudo.length} chars`);
    }
  ...
  $content('style').remove();
    ...
    $content('div, p, span, a, button').each((i, el) => {
      ...
      if (
        ...
        console.log(`[Olhar Digital Test] Removendo compartilhamento: ${text.substring(0, 30)}...`);
        $el.remove();
        return;
      }
      ...
      if (
        ...
        console.log(`[Olhar Digital Test] Removendo navegação: ${text.substring(0, 30)}...`);
        $el.remove();
        return;
      }
      ...
      if (
        ...
        console.log(`[Olhar Digital Test] Removendo loading: ${text.substring(0, 30)}...`);
        $el.remove();
        return;
      }
      ...
      if (
        ...
        console.log(`[Olhar Digital Test] Removendo elemento do Olhar Digital: ${text.substring(0, 30)}...`);
        $el.remove();
      }
    });
  ...
  $content('div:empty, p:empty, span:empty, section:empty, article:empty, figure:empty').each((i, el) => {
    const $el = $content(el);
    ...
    if ($primeiraImg.length > 0) {
      ...
      if (src && isArticleImage && isNotAd) {
        ...
        console.log(`[Olhar Digital Test] 📸 Primeira imagem extraída como destaque: ${finalSrc.substring(0, 50)}...`);
    ...
    $content('iframe').each((i, el) => {
      ...
      if (src.includes('youtube.com') || src.includes('youtu.be')) {
        ...
        console.log(`[Olhar Digital Test] Processando vídeo YouTube: ${src.substring(0, 50)}...`);
      }
        ...
        console.log(`[Olhar Digital Test] Processando vídeo: ${src.substring(0, 50)}...`);
      }
      else {
        $iframe.remove();
      }
    ...
    $content('a').each((i, el) => {
      ...
      if (href.includes('youtube.com/watch') || href.includes('youtu.be/')) {
        ...
        if (videoId) {
          ...
          console.log(`[Olhar Digital Test] Convertendo link YouTube para embed: ${videoId}`);
        }
      }
    });
    ...
    $content('h2, h3').each((i, el) => {
      ...
      if (text.length > 3 &&
        ...
        if ($heading.is('h2')) {
          ...
          console.log(`[Olhar Digital Test] 📝 Subtítulo H2 encontrado: ${text.substring(0, 40)}...`);
        } else if ($heading.is('h3')) {
          $heading.addClass('news-heading-3 news-subtitle');
          console.log(`[Olhar Digital Test] 📝 Subtítulo H3 encontrado: ${text.substring(0, 40)}...`);
        }
      }
    });
  ...
  const bodyHtml = `<div class="news-content">${cleanHtml}</div>`;
    ...
    console.log(`[Olhar Digital Test] ✅ Título: ${titulo.substring(0, 60)}...`);
    console.log(`[Olhar Digital Test] ✅ Resumo: ${resumo.substring(0, 60)}...`);
    console.log(`[Olhar Digital Test] ✅ Imagem: ${primeiraImagem ? 'SIM' : 'NÃO'}`);
    console.log(`[Olhar Digital Test] ✅ Conteúdo: ${bodyHtml.length} chars`);
    ...
    console.error(`[Olhar Digital Test] ❌ Erro no scraping: ${error.message}`);
    return null;
  }
}
...
async function noticiaExiste(url, slug) {
  try {
    ...
    if (!response.ok) {
      console.log(`[Olhar Digital Test] Erro ao verificar notícia existente: ${response.status}`);
      return false;
    }
    ...
    console.log(`[Olhar Digital Test] Erro ao verificar notícia existente: ${error.message}`);
    return false;
  }
}
...
async function createNoticia(item, url, data_publicacao) {
  ...
  console.log(`[Olhar Digital Test] 📝 Criando notícia...`);
  console.log(`[Olhar Digital Test] - Categoria: ${item.categoria}`);
  console.log(`[Olhar Digital Test] - URL Imagem: ${item.imagem ? 'SIM' : 'NÃO'}`);
  console.log(`[Olhar Digital Test] - Conteúdo: ${item.conteudo.length} chars`);


  // Verificar se já existe
  const existe = await noticiaExiste(url, slug);
  if (existe) {
    console.log(`[Olhar Digital Test] Notícia já existe, pulando: ${slug}`);
    return 'skipped';
  }
    ...
    if (data_publicacao) {
      try {
        ...
        console.log('[Olhar Digital Test] Erro ao parsear data, usando data atual');
      }
    }
    ...
    if (response.ok) {
      ...
      console.log(`[Olhar Digital Test] ✅ Notícia criada com sucesso! ID: ${result.data.id}`);
      return true;
    } else {
      const errorText = await response.text();
      console.log(`[Olhar Digital Test] ❌ Erro ao criar notícia: ${errorText}`);
      return false;
    }
  } catch (error) {
    console.error('[Olhar Digital Test] ❌ Erro ao criar notícia:', error);
    return false;
  }
}
