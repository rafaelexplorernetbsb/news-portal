...
export default function Home() {
  ...
  const [categoriasComNoticias, setCategoriasComNoticias] = useState<
    Array<{ categoria: any; noticias: Noticia[] }>
  >([]);
  ...
  useEffect(() => {
    async function loadNoticias() {
      try {
          ...
          Array.from(categoriasUnicas)
            ...
            .map(async (categoriaNome) => {
              ...
              if (categoriaNome.toLowerCase().includes('cultura')) {
                noticiasCategoria = await getNoticiasPorCategoriaEspecifica(
                  'Cultura',
                  12
                );
              } else {
        ...
        const categoriasComNoticiasFiltradas = categoriasComNoticiasData.filter(
        ...
        <>
                  ...
                  >
                        ...
                        <span className="bg-[#db0202] text-white text-xs font-bold px-2 py-1 rounded uppercase tracking-wide">
                          {capitalizarCategoria(
                            typeof noticiasDestaque[0].categoria === 'string'
                              ...
                              : typeof noticiasDestaque[0].categoria ===
                                    'object' &&
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center justify-between text-sm text-gray-500">
                        <span>
                          {formatarData(noticiasDestaque[0].data_publicacao)}
                        </span>
                      </div>
                    </div>
                  </Link>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            <div className="lg:col-span-3">
              ...
              {categoriasComNoticias.map(
                ({ categoria, noticias }, categoriaIndex) => (
                  <section key={categoria.id} className="mb-8">
                    <div className="flex items-center mb-4">
                      ...
                      <div
                        ...
                        style={{
                          clipPath: 'polygon(42% 0, 95% 0, 55% 100%, 0 100%)',
                        }}
                      />
                      <div
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                      {noticias.slice(0, 12).map((noticia, index) => (
                        <NoticiaCard
                          key={`${categoria.id}-${noticia.id}-${index}`}
                          noticia={noticia}
                          featured
                        />
                      ))}
